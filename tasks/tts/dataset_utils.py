    ...
    def __getitem__(self, index):
        ...
        if hparams['use_spk_embed']:
            try:
                ...
                spk_emb = np.load("/workspace/dataset/spk/{}.npy".format(item['item_name']))
                sample["spk_embed"] = torch.Tensor(spk_emb)
...
class DiffProsodyDataset(FastSpeechWordDataset):
    def __init__(self, prefix, shuffle=False, items=None, data_dir=None):
        super().__init__(prefix, shuffle=False, items=None, data_dir=None)
    def __getitem__(self, index):
        sample = super().__getitem__(index)

        item_name = sample["item_name"]
        item = np.load(os.path.join(self.hparams["tts_model"], "lpvs", item_name + ".npz"))
        
        sample["lpv"] = torch.FloatTensor(item["lpv"])
        lpv_idx = np.array(item["lpv_idx"])
        sample["lpv_idx"] = torch.from_numpy(lpv_idx).long()
        lpv_in = lpv_idx[:-1]
        lpv_out = lpv_idx[1:]

        sample["lpv_in"] = torch.from_numpy(lpv_in).long()
        sample["lpv_out"] = torch.from_numpy(lpv_out).long()
    ...
    def collater(self, samples):
        ...
        lpvs = collate_1d_or_2d([s['lpv'] for s in samples], 0)
        lpvs_idx = collate_1d_or_2d([s['lpv_idx'] for s in samples], 0)
        lpvs_in = collate_1d_or_2d([s['lpv_in'] for s in samples], 0)
        lpvs_out = collate_1d_or_2d([s['lpv_out'] for s in samples], 0)

        batch['lpvs_idx'] = lpvs_idx
        batch['lpvs_in'] = lpvs_in
        batch['lpvs_out'] = lpvs_out
        batch['lpvs'] = lpvs
